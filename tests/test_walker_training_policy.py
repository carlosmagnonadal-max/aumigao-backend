...
def test_grace_period_respected(tmp_path, monkeypatch):
    ...
    assert during.deadline == datetime(2026, 9, 22, 0, 0)
    assert policy.get_enforcement(now=datetime(2026, 9, 22, 0, 0)).blocking is True
...
def test_invalid_grace_days_uses_7(tmp_path, monkeypatch):
    ...
    assert policy.get_enforcement(now=datetime(2026, 9, 22)).blocking is True
